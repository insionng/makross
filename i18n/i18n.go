...
func prepareOptions(options []Options) Options {
	...
	sec := makross.Config().Section(opt.Section)
...
func I18n(options ...Options) makross.Handler {
	...
	return func(ctx *makross.Context) error {
		...
		if len(lang) == 0 {
			tags, _, _ := language.ParseAcceptLanguage(string(ctx.Request.Header.Get("Accept-Language")))
		...
		if !hasCookie {
			...
			cookie.Name = "lang"
			cookie.Value = curLang.Lang
			cookie.Expires = time.Now().Add(365 * 24 * time.Hour)
			cookie.Path = ("/" + strings.TrimPrefix(opt.SubURL, "/"))
		...
		ctx.Set("RestLangs", restLangs)

		if opt.Redirect && isNeedRedir {
			ctx.Redirect(opt.SubURL + ctx.Request.URL.String()[:strings.Index(ctx.Request.URL.String(), "?")])
		}
		return ctx.Next()
	}
}
