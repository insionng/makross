...
func (p *proxyRawHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		he := makross.NewHTTPError(http.StatusBadGateway, fmt.Sprintf("proxy raw, dial error=%v, url=%s", err, p.t.URL))
		p.c.HandleError(he)
		return
	}
	defer out.Close()
	...
	err = r.Write(out)
	if err != nil {
		he := makross.NewHTTPError(http.StatusBadGateway, fmt.Sprintf("proxy raw, request header copy error=%v, url=%s", p.t.URL, err))
		p.c.HandleError(he)
		return
	}
...
func proxyRaw(t *ProxyTarget, c *makross.Context) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			he := makross.NewHTTPError(http.StatusBadGateway, fmt.Sprintf("proxy raw, dial url=%s, error=%v", t.URL, err))
			c.HandleError(he)
			return
		}
		defer out.Close()
		...
		err = r.Write(out)
		if err != nil {
			he := makross.NewHTTPError(http.StatusBadGateway, fmt.Sprintf("proxy raw, request header copy url=%s, error=%v", t.URL, err))
			c.HandleError(he)
			return
		}
...
func Proxy(config ProxyConfig) makross.Handler {
	...
	return func(c *makross.Context) (err error) {
		...
		if req.Header.Get(makross.HeaderXRealIP) == "" {
			req.Header.Set(makross.HeaderXRealIP, c.RealIP())
		}
		if req.Header.Get(makross.HeaderXForwardedProto) == "" {
			req.Header.Set(makross.HeaderXForwardedProto, c.Scheme())
		}
		if c.IsWebSocket() && req.Header.Get(makross.HeaderXForwardedFor) == "" { // For HTTP, it is automatically set by Go HTTP reverse proxy.
			req.Header.Set(makross.HeaderXForwardedFor, c.RealIP())
		}
