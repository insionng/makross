...
// attacks.
func Secure() makross.Handler {
	return SecureWithConfig(DefaultSecureConfig)
}

// SecureWithConfig returns a Secure middleware with config.
// See: `Secure()`.
func SecureWithConfig(config SecureConfig) makross.Handler {
	// Defaults
	...
	return func(c *makross.Context) error {
		...
		if config.XSSProtection != "" {
			res.Header().Set(makross.HeaderXXSSProtection, config.XSSProtection)
		}
		if config.ContentTypeNosniff != "" {
			res.Header().Set(makross.HeaderXContentTypeOptions, config.ContentTypeNosniff)
		}
		if config.XFrameOptions != "" {
			res.Header().Set(makross.HeaderXFrameOptions, config.XFrameOptions)
		}
		if (c.IsTLS() || (string(req.Header.Get(makross.HeaderXForwardedProto)) == "https")) && config.HSTSMaxAge != 0 {
			subdomains := ""
